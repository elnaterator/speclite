...
description: >
  ...
  and its roadmap item, mode and any active halt, the branch's open PR, and a dry-run
  of what speclite-run would do next. Strictly read-only, safe to run anytime. Use when the
  user says "speclite status", "where am I", "show pipeline state", "what would run do", or
  invokes /speclite-status.
---

Report the current state of the speclite pipeline. This skill is **strictly read-only**: it
reads roadmap status + git state + mode markers and prints a summary. It never writes a
file, mutates git, commits/pushes/opens a PR, or touches `.halt`. Safe to run any
time as a daily check or to preview the loop before committing to a loop mode.
...
3. **Compute the next step as a dry-run.** Apply the `speclite-run` decision table to the
   ...
   - On `<type>/<NNN>-…` branch, item `PLANNED`/`WIP` → would run `speclite-build`.
   - On branch, item `BUILT` → would run `speclite-review`, then halt at the pre-commit gate
     (`default`/`semi-auto`) or run `speclite-ship` then halt after the PR (`full-auto`).
   - Roadmap all `SHIPPED` / no backlog item → nothing to do.
   - Dirty tree (unrelated changes), off-trunk-when-expected, branch without `<NNN>`, or
     missing item for the branch → would halt and ask.

4. **Report** a compact summary:
   - Roadmap items grouped by status with counts (backlog / PLANNED / WIP / BUILT / SHIPPED).
   - Current branch and the roadmap item it maps to (or "not tied to an item").
   - Mode, and the halt marker + reason if present.
   - Open PR (number/title/state/url) or "none".
   - **Next:** the dry-run action from Step 3, labeled clearly as a preview (suggest running
     `/speclite-run` to actually advance).
