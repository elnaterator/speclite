...
description: >
  Review the current branch's diff against its plan's acceptance criteria and the project
  rules, flag drift (missing criteria, scope creep, unverified claims), and report findings.
  Runs after speclite-build and before the commit gate. Use when the user says "speclite
  review", "review this item", "check the diff against the plan", or invokes /speclite-review.
  Also invoked conditionally by the dispatcher (speclite-run).
---
...
Manual invocation always runs a **full** review. Dispatcher invocation is **conditional**: the
...
3. **Need-review gate** — *dispatcher dispatch only; a manual run always reviews, skip to Step 4.*
   ...
   - the project rules opt into always-review.
   ...
   session (one line, same transparency spirit as `.halt`) and exit clean — do **not**
...
4. **Review.** For each acceptance criterion, judge whether the diff satisfies it; cite the
   ...
   - **unverified claims** — item marked BUILT but a criterion has no supporting change/test;
   - **rules conflicts** — anything the diff does that the project rules forbid.
...
5. **Verdict handling by mode.**
   ...
   - **semi-auto** → surface findings. On **FAIL**, write `.halt`
     (`review failed — <NNN>: <one-line summary>`) so the pipeline stops before commit. On
   ...
   - **full-auto** → on **FAIL**, **auto-fix** the flagged issues using the review's
     ...
     **3 attempts**. On PASS, no halt (dispatcher proceeds to commit). On non-convergence after
     the cap, write `.halt`
     (`review did not converge after 3 attempts — <NNN>`) rather than commit broken work.

6. **Defensive posture.** If pass/fail cannot be determined (ambiguous criteria, can't map
   diff to plan) or auto-fixes don't converge, **halt** (write `.halt` with the
   reason) instead of rubber-stamping. Never mark anything BUILT or commit from this skill.

7. Report: the verdict, findings + recommendations, whether review was skipped (and why), and
   in full-auto how many fix attempts ran. Suggest `/speclite-ship` next when PASS in a
   manual/semi-auto run.

## Boundaries

- Report-only by default. Edits code **only** in `full-auto`, and only to fix flagged findings.
- Never commits, pushes, opens/merges a PR, or marks the roadmap item BUILT/SHIPPED.
- Reuses `.halt` for halts; introduces no new persistent state file.
