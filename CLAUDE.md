...
`speclite-roadmap` sits **outside** the pipeline: it is the only skill that edits roadmap *text* (investigate an idea → `research/<topic>.md`, add items minting the next `<NNN>` on trunk, refine backlog-only items). No branch/plan/PR by default — commits `chore(roadmap): …` on trunk and pushes, with a `rules.md` opt-in for the branch+PR path. Never dispatched by `speclite-run`.
...
- **Mode markers live in `specs/lite/`** and are git-ignored (`speclite-init` writes `specs/lite/.gitignore`): `.mode` (contents = `default`/`semi-auto`/`full-auto`, user intent; absent ⇒ `default`) and `.halt` (transient stop signal, reason as contents). `speclite-run` clears any stale `.halt` at start, writes it on every halt path; Stop hook allows stop when mode is `default`/absent OR halt present, blocks-and-continues only when mode is a loop mode + no halt. **Setting a loop mode via `speclite-mode` writes `.halt` (not clears it)** so the loop never self-starts — only an explicit `/speclite-run` (which clears `.halt`) begins advancing. `speclite-status` is strictly read-only and never touches `.halt`, so checking status also never kicks the loop. This makes every termination explicit (no infinite loop). `default`/`semi-auto` **never** auto-commit/push/PR; **full-auto** deliberately does (commit + push + open PR) but always halts after the PR — never merges.
